...
class SewaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $waktu_balik = Carbon::parse($waktu_pjm)->addHours($durasi)->format('Y-m-d H:i:s');

        try {
            $sewa = DB::transaction(function () use ($mobilName, $supirName, $durasi, $waktu_pjm, $waktu_balik, $nama, $nohp, $alamat, $jaminan, $customer) {
                ...
                if ($this->overlapExists('mobil', $mobilName, $waktu_pjm, $waktu_balik) ||
                    $this->overlapExists('supir', $supirName, $waktu_pjm, $waktu_balik)) {
                    throw ValidationException::withMessages(['mobil' => 'Kendaraan atau supir sudah dipesan pada rentang waktu tersebut.']);
                }

                $hari = max(1, (int) ceil($durasi / 24));
                $totalBiaya = ((int) $mobil->sewa + (int) $supir->sewa) * $hari;
    ...
    public function calculatePrice(Request $request)
    {
        ...
        $supir = $request->input('supir') ? Supir::where('nama', $request->input('supir'))->first() : null;
        $durasi = max(1, (int) ($request->input('durasi') ?: 24));
        $hari = (int) ceil($durasi / 24);

        $total = ((int) ($mobil->sewa ?? 0) + (int) ($supir->sewa ?? 0)) * $hari;
